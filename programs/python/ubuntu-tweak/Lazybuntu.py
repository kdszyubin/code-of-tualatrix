...
class ToolPage:
    ...
    def get_widget(self):
        ...
        list=gtk.ListStore(gobject.TYPE_BOOLEAN, gobject.TYPE_STRING, gobject.TYPE_PYOBJECT)
        view=gtk.TreeView()
    ...
    def clear_selected(self):
        list = self.get_widget().get_model()
        it = list.get_iter_first()
        while True:
            if it == None or it == 0:
                break
            list.set(it, 0, False)
...
'''
# 鎖住 /var/lib/dpkg/lock 會連帶造成 apt-get 以及 dpkg 不能使用
# 改 lock /root/.synaptic/lock 比較理想，只避免 synaptic / update manager 執行
# FIXME: better solution?
lock_file = '/root/.synaptic/lock'
lock_fd = -1
def on_hup():
    print 'HUP!'
    # orz... How to prevent the SIGHUP sent by update manager?

def acquire_lock():
    import fcntl, signal

    signal.signal( 1, on_hup )

    lock_fd = os.open( lock_file, os.O_RDWR | os.O_CREAT | os.O_TRUNC, 0640 )
    if lock_fd >= 0:
        try:
            ...
            pass
    show_error( '偵測到其他套件管理程式在執行，請先關閉。', '無法執行 Lazybuntu' )
    exit(1)

def release_lock():
    if lock_fd >= 0:
        fcntl.lockf( lock_fd, LOCK_UN )
        os.close( lock_fd )
        os.unlink( lock_file )
'''
...
#    acquire_lock()
    ...
    #Add our own fallback icons
    #gtk.icon_theme_get_default().append_search_path( './icons' )
